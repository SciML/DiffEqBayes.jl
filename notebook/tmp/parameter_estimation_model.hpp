...
stan::io::program_reader prog_reader__() {
    ...
    reader.add_event(0, 0, "start", "/home/vaibhav/DiffEqBayes.jl/notebook/tmp/parameter_estimation_model.stan");
    reader.add_event(39, 39, "end", "/home/vaibhav/DiffEqBayes.jl/notebook/tmp/parameter_estimation_model.stan");
    return reader;
}
...
public:
    ...
    void transform_inits(const stan::io::var_context& context__,
        ...
        try {
            writer__.scalar_lub_unconstrain(0.5,2.5,theta1);
        } catch (const std::exception& e) { 
        ...
        try {
            writer__.scalar_lub_unconstrain(0.0,2.0,theta2);
        } catch (const std::exception& e) { 
        ...
        try {
            writer__.scalar_lub_unconstrain(1.0,4.0,theta3);
        } catch (const std::exception& e) { 
    ...
    T__ log_prob(vector<T__>& params_r__,
        ...
        if (jacobian__)
            theta1 = in__.scalar_lub_constrain(0.5,2.5,lp__);
        else
            theta1 = in__.scalar_lub_constrain(0.5,2.5);

        T__ theta2;
        (void) theta2;  // dummy to suppress unused var warning
        if (jacobian__)
            theta2 = in__.scalar_lub_constrain(0.0,2.0,lp__);
        else
            theta2 = in__.scalar_lub_constrain(0.0,2.0);

        T__ theta3;
        (void) theta3;  // dummy to suppress unused var warning
        if (jacobian__)
            theta3 = in__.scalar_lub_constrain(1.0,4.0,lp__);
        else
            theta3 = in__.scalar_lub_constrain(1.0,4.0);
        ...
        try {
            {
                ...
                if (get_base1(theta,1,"theta",1) < 0.5) lp_accum__.add(-std::numeric_limits<double>::infinity());
                else if (get_base1(theta,1,"theta",1) > 2.5) lp_accum__.add(-std::numeric_limits<double>::infinity());
                else lp_accum__.add(-log_diff_exp(normal_cdf_log(2.5, 1.5, 0.01), normal_cdf_log(0.5, 1.5, 0.01)));
                current_statement_begin__ = 34;
                ...
                if (get_base1(theta,3,"theta",1) < 1.0) lp_accum__.add(-std::numeric_limits<double>::infinity());
                else if (get_base1(theta,3,"theta",1) > 4.0) lp_accum__.add(-std::numeric_limits<double>::infinity());
                else lp_accum__.add(-log_diff_exp(normal_cdf_log(4.0, 3.0, 0.01), normal_cdf_log(1.0, 3.0, 0.01)));
                current_statement_begin__ = 34;
    ...
    void write_array(RNG& base_rng__,
        ...
        double theta1 = in__.scalar_lub_constrain(0.5,2.5);
        double theta2 = in__.scalar_lub_constrain(0.0,2.0);
        double theta3 = in__.scalar_lub_constrain(1.0,4.0);
