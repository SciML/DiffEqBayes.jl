...
bayesian_result = dynamichmc_inference(
    prob1, Tsit5(), t, data, (Normal(1.5, 1),),
...
for i in 1:3:length(data)
    weights_[i] = 0
    data[i] = 1.0e20 # to test that those points are indeed not used
end
...
@test_broken bayesian_result = dynamichmc_inference(
    prob1, Tsit5(), likelihood,
    [truncated(Normal(1.5, 1), 0, 2)],
    as((a = asℝ₊,))
)
@test_broken mean(bayesian_result[1][1]) ≈ 1.5 atol = 1.0e-1
