...
@test mean(get(bayesian_result.chains, :theta_1)[1]) ≈ 1.5 atol = 3.0e-1

# Test norecompile
bayesian_result2 = stan_inference(
    prob1, alg, t, data, priors;
    stanmodel = bayesian_result.model, sample_kwargs, likelihood = Normal
)

@test mean(get(bayesian_result2.chains, :theta_1)[1]) ≈ 1.5 atol = 3.0e-1
...
@test mean(get(bayesian_result.chains, :theta_1)[1]) ≈ 1.0 atol = 3.0e-1
@test mean(get(bayesian_result.chains, :theta_2)[1]) ≈ 1.0 atol = 3.0e-1
@test mean(get(bayesian_result.chains, :theta_3)[1]) ≈ 1.5 atol = 3.0e-1
...
bayesian_result = stan_inference(
    prob1, alg, t, data, priors; solve_kwargs, sample_kwargs,
    likelihood = Normal, sample_u0 = true
)

@test mean(get(bayesian_result.chains, :theta_1)[1]) ≈ 1.0 atol = 3.0e-1
@test mean(get(bayesian_result.chains, :theta_2)[1]) ≈ 1.5 atol = 3.0e-1
...
data = convert(Array, randomized)
priors = [
    truncated(Normal(1.5, 0.01), 0.5, 2), truncated(Normal(1.0, 0.01), 0.5, 1.5),
    truncated(Normal(3.0, 0.01), 0.5, 4), truncated(Normal(1.0, 0.01), 0.5, 2),
]

bayesian_result = stan_inference(
    prob1, alg, t, data, priors; sample_kwargs = Dict(
        :num_samples => 100,
        :num_warmups => 500
    ),
    vars = (DiffEqBayes.StanODEData(), InverseGamma(4, 1))
)
@test mean(get(bayesian_result.chains, :theta_1)[1]) ≈ 1.5 atol = 1.0e-1
@test mean(get(bayesian_result.chains, :theta_2)[1]) ≈ 1.0 atol = 1.0e-1
@test mean(get(bayesian_result.chains, :theta_3)[1]) ≈ 3.0 atol = 1.0e-1
@test mean(get(bayesian_result.chains, :theta_4)[1]) ≈ 1.0 atol = 1.0e-1
