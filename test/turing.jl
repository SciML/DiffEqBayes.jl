...
bayesian_result = turing_inference(
    prob1, Tsit5(), t, data, priors; syms = [:a], sample_args = (num_samples = 500,)
)
...
bayesian_result = turing_inference(
    prob1, Rosenbrock23(), t, data, priors; syms = [:a],
    sample_args = (num_samples = 500,)
)

# --- test Multithreaded sampling
println("Multithreaded case")
result_threaded = turing_inference(
    prob1, Tsit5(), t, data, priors;
    syms = [:a], sample_args = (
...
data = convert(Array, randomized)
priors = [
    truncated(Normal(1.5, 0.01), 0, 2), truncated(Normal(1.0, 0.01), 0, 1.5),
    truncated(Normal(3.0, 0.01), 0, 4), truncated(Normal(1.0, 0.01), 0, 2),
]

bayesian_result = turing_inference(
    prob2, Tsit5(), t, data, priors;
    syms = [:a, :b, :c, :d], sample_args = (num_samples = 500,)
)
...
bayesian_result = turing_inference(
    s_prob, DynamicSS(Tsit5()),
    nothing, data, priors;
    syms = [:α],
    solve_kwargs = Dict(:abstol => 1.0e-4, :reltol => 1.0e-3, :maxiters => 1.0e6),
    sample_args = (num_samples = 500,)
)
